    ...
    <c:if test="${total_page != 1}">
        <div class="pagination">
            <c:if test="${current_page != 1}">
                <a href="${pageContext.request.contextPath}?page=${current_page-1}&sort=${sort}&size=${size}">«</a>
                <a href="${pageContext.request.contextPath}?page=1&sort=${sort}&size=${size}">1</a>
            </c:if>
            <c:if test="${current_page == 1}">
                <a class="inactive">«</a>
                <a class="active">1</a>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:if test="${p > 1 && p < total_page}">
                                <a href="${pageContext.request.contextPath}?page=${p}&sort=${sort}&size=${size}">${p}</a>
                            </c:if>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
            ...
            <c:if test="${current_page != total_page}">
                <a href="${pageContext.request.contextPath}?page=${total_page}&sort=${sort}&size=${size}">${total_page}</a>
                <a href="${pageContext.request.contextPath}?page=${current_page + 1}&sort=${sort}&size=${size}">»</a>
            </c:if>
            <c:if test="${current_page == total_page}">
